...
class TEST:
    ...
    def _loss_objects(self, im, scores, boxes, image_name, thresh, nms_tresh=0.3):
        """All detected objects on photo(with threshold)"""

    #    print('\nimg name: ', image_name)
        gt_boxes = self._load_annotations(xml_name=image_name[:-3] + 'xml')
        ...
        for cls_ind, cls in enumerate(CLASSES[1:]):
            ...
            for bbox in dets:
                for gt_bbox in gt_boxes_class:
                    iou = self._bb_intersection_over_union(bbox, gt_bbox)
                    if iou > 0.4:
    ...
    def _test(self, sess, net, image_name, thresh):
        ...
        return self._loss_objects(im, scores, boxes, image_name, thresh=thresh, nms_tresh=NMS_THRESH)

    def testing(self, end='test', thresh=0.9):
            ...
            for ind, im_name in enumerate(test_images):
                loss += self._test(self._sess, self._net, im_name[:-1] + '.jpg', thresh=thresh)
                if ind % 20 == 0:
